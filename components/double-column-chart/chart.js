...
function drawColumn(series, opts, config, context) {
  ...
  series.lineData[0].data.forEach(function (item, index) {
    ...
    // if (distance == 0 || Math.abs(areaData.x - config.touchDetail.x - Math.abs(opts._scrollDistance_)) < distance) {
    //   console.log('-------------')
    //   console.log(config.touchDetail.x + Math.abs(opts._scrollDistance_))
    //   console.log('-------------')
    // // if (distance == 0 || Math.abs(areaData.x - Math.abs(opts._scrollDistance_)) < distance) {
    ...
    var ljj_realDistance = config.touchDetail.x + Math.abs(opts._scrollDistance_);
    console.log('ljj_realDistance---' + ljj_realDistance)
    console.log(DataAxisX[index].x)
    
    var ljj_itemDistance = leftOffset + 78 * (index);
    if (Math.abs(ljj_realDistance - DataAxisX[index].x) < 40) {
  ...
  if (series.lineData != null && series.lineData != undefined) {
    ...
    lines.forEach(function (items, index) {
      ...
      context.stroke();
      //-----------------------------填充
      context.lineTo(LastX, startPoint.y);
      context.lineTo(startPoint.x, startPoint.y);
      //连接并填充区域
      context.closePath();
      context.setFillStyle('#00BFFF');
  ...
  if (config.touchDetail.isShow) {
    //console.log(opts._scrollDistance_)
    console.log('1----------')
    //debugger
    context.beginPath();
    ...
    if ((realTouch.x + config.touchDetail.padd * 2 + config.touchDetail.width) < yAxisMaxPoint) {
      ...
      context.fillRect(Math.abs(opts._scrollDistance_) + config.touchDetail.x + config.touchDetail.padd * 2, topOffset - axisPadd.top, config.touchDetail.width, config.touchDetail.height);
    } else {
      console.log('2----------')
      if (realTouch.title) {
        context.fillRect(Math.abs(opts._scrollDistance_) + config.touchDetail.x + config.touchDetail.padd * 2, topOffset - axisPadd.top, config.touchDetail.width, config.touchDetail.height);
    ...
    if ((realTouch.x + config.touchDetail.padd * 2 + config.touchDetail.width) < yAxisMaxPoint) {
      ...
      realTouch.title.forEach(function (item, index) {
        context.fillText(item, Math.abs(opts._scrollDistance_) + config.touchDetail.x + config.touchDetail.padd * 2, topOffset - axisPadd.top + config.touchDetail.padd + config.touchDetail.lineSpacingExtra * index + config.touchDetail.fontSize * (index + 1));
 ...
 * 计算有效的滑动距离
 */
function calValidDistance2(distance, currentOffset, config) {
  var padd = config.padd;
  var axisMargin = config.axisMargin;
  var axisPadd = config.axisPadd;
